...
type ImageTmp struct {
	userID string `db:"jia_user_id"`
	isuID  string `db:"jia_isu_uuid"`
	image  []byte `db:"image"`
}
...
func createIconsFromRDB() error {
	...
	for _, img := range images {
		out, _ := os.Create(getIsuIconPath(img.userID, img.isuID))
...
func postInitialize(c echo.Context) error {
	...
	if err := createIconsFromRDB(); err != nil {
		c.Logger().Errorf("initialize error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
	}
...
func getIsuList(c echo.Context) error {
	...
	isuList := []IsuForGetIsuListFromDB{}
	err = tx.Select(
		&isuList,
		"WITH con AS ( SELECT `latest_time`, `is_sitting`, `condition`, `message`, `created_at`, isu_condition.jia_isu_uuid FROM isu_condition JOIN ( SELECT jia_isu_uuid, max(timestamp) as latest_time from isu_condition group by jia_isu_uuid ) newcon on isu_condition.jia_isu_uuid = newcon.jia_isu_uuid where isu_condition.timestamp = newcon.latest_time ) SELECT id, isu.jia_isu_uuid, isu.name, isu.character, latest_time as timestamp, is_sitting, `condition`, `message`, c.created_at, updated_at FROM isu LEFT OUTER JOIN con as c ON isu.jia_isu_uuid = c.jia_isu_uuid WHERE jia_user_id = ? ORDER BY id DESC",
		jiaUserID)
	if err != nil {
		c.Logger().Errorf("db error: %v", err)
		return c.NoContent(http.StatusInternalServerError)
